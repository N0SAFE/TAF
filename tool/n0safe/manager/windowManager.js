...
class BlankWindow {
    constructor(childWindow) {
        this.window = childWindow
    }

    add(str, header) {
        if (this.window.firstOpen || this.window.opened.closed) {
            this.window.open()
        }
        setTimeout(function() {
            str = "<div>" + (header ? "<h2 style='text-align: center;'>" + header + "</h2>" : "") + str + "</div>"
