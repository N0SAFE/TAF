...
export const importScript = async(path) => {
    ...
    if (!TAF.io.EXPORT.get(path)) {
        TAF.io.EXPORT.set(path, {})
    }
    return {
        script: await
        import ((isValidURL(path) ? "" : dirname(
            import.meta.url, 4) + "/") + path),
        module: TAF.io.EXPORT.get(path),
...
export const addInObject = function() {
    if (arguments.length < 3)
        throw new Error("must be called with 3 arguments but " + arguments.length + " found")
    ...
    function recFunc(OBJECT, suit, value) {
        if (suit.length > 1) {
            if (!Array.from(Object.keys(OBJECT)).includes(suit[0]) || typeof OBJECT[suit[0]] !== 'object') {
                OBJECT[suit[0]] = {}
            }
...
export function searchInObject() {
    ...
    let recFunc = (OBJECT, suit) => {
        if (suit.length > 0) {
            ...
            return new types.notFounds.searchInObject()
        }
        return OBJECT
    }
...
export function removeInObject() {
    if (arguments.length < 2)
        throw new Error("must be called with 2 arguments but " + arguments.length + " found")
    let OBJECT = arguments[0]
    let suit = arguments[1]
...
/**
 * Try to execute the Try function, if it fails, execute the Catch function, and then execute the
 * Finally function.
 * @param obj - {Try, Catch, Finally}
 * @returns The return value of the Try function.
 */
export function Try({ try: Try, catch: Catch, finally: Finally }) {
    let ret
...
export class StackContainer {
    constructor(metaScriptUrl) {
        /* Parsing the error stack and avoiding the file that is importing the error stack. */
        this.stack = TAF.module.error.ErrorStackParser.parseWithAvoid([{
            fileName: metaScriptUrl
        }, {
...
/**
 * "It takes a function and an array of arguments, and returns an array of
 * the results of calling the function with each of the arguments."
 * 
 * Here's a more detailed explanation:
 * 
 * The function takes two arguments:
 * 
 * func: a function that takes any number of arguments
 * array: an array of arrays, where each array is a set of arguments to be passed to the function
 * The function returns a promise that resolves to an array of the results of calling the function with
 * each of the arguments
 * @param func - The function you want to call
 * @param array - an array of arrays, each of which contains the arguments to be passed to the function
 * @return an array
 */
export const multiCall = (func, array) => array.forEach((args) => func(...args))


/**
 * "It takes a function and an array of arguments, and returns a promise that resolves to an array of
 * the results of calling the function with each of the arguments."
 * 
 * Here's a more detailed explanation:
 * 
 ...
 * @param func - The function you want to call
 * @param array - an array of arrays, each of which contains the arguments to be passed to the function
 * @return a promise that resolve by an array
 */
export const asyncMultiCall = (func, array) => Promise.all(array.map(async(args) => await func(...args)))
...
/**
 * If all the arguments are equal to each other, return true, otherwise return false.
 */
...
// this is class that can be extends to create a notFound class that can search the instance by making instaneof this class
// ! doc to do
export class notFound {}

// the types is the class to be used to communicate with the function return state and store unique type (like in searchInObject when nothing is find the function return a specifique notFound class called searchInObject extends of notFound, this class is the same as notFound but it can focus on wich function is the parent of this returnement state)
export const types = {
    notFounds: {
        default: notFound,
